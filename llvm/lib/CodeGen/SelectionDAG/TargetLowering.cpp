...
SDValue TargetLowering::expandCONVERT_TO_ARBITRARY_FP(SDNode *Node,
  ...
  // negative input saturates to zero (the smallest representable value), or is
  // poison when not saturating. Applied before the NaN case so that a negative
  // NaN still produces the NaN encoding.
  if (!DstHasSign) {
    SDValue IsNegative =
